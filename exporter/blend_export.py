...
resource_folder = output_folder[len(game_root):]
...
def export_selected_objects(filepath, model):
...
def render_icon(model):
    ...
    scene.render.filepath = output_folder + "/" + model["name"] + "_icon.png"
...
for collection in scene.collection.children.values(): 
    ...
    if collection.name.find("Collection")!=0 and collection.name[0] != "_":
        ...
        model["res_path"] = resource_folder+"/"+model_name+"."+model["properties"]["format"]
